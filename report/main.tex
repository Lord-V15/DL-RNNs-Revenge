...
\textbf{Induction Heads.} Sequences of the form \texttt{[random][5-char pattern P][random][P[:4]]}, where the model must predict the 5th character of $P$ given its first 4 characters reappearing later in the sequence. Generated at $T \in \{128, 256, 2048\}$ with one pattern per sequence. This tests content-based retrieval: the model must use the current context as a \emph{query} to locate where the pattern appeared previously and recall its final character. Metric: accuracy at the prediction position (the 5th character).
...
Figure~\ref{fig:phase} reveals a striking training dynamic: the Transformer's induction-long accuracy remains at random baseline (0.04) for 7,000 steps, then rises sharply (0.48 at step 8K, 1.0 at step 9K), completing the transition in ${\sim}$2,000 steps. This is consistent with ``grokking'' \citep{power2022grokking} and the circuit formation hypothesis \citep{olsson2022context}: the attention heads abruptly snap into an induction circuit configuration. This implies the Transformer's failure at $T{=}2048$ copy likely reflects insufficient training (5K steps) for the phase transition to occur, rather than an architectural limitation; given more steps, we predict the copy circuit would similarly emerge.
